...
- UPDATE MAY 2020: A default index.ndx is not generated by CHARMM-GUI. Make sure to generate
    by yourself using "gmx make_ndx -f step3_input.gro -o index.ndx
...
version = "charmm2hpc.py v0.7 07/2020 for Python 3+"
...
pull_coord1_groups      = 1 2
pull_coord1_start       = yes       ; define initial COM distance > 0
pull_coord1_rate        = %.8f      ; 0.1 = 0.01 nm per ps = 10 nm per ns
pull_coord1_k           = %d      ; kJ mol^-1 nm^-2
pull-nstxout            = 10
pull-nstfout            = 1
;use -pf and -px names after the filename for data analysis
''' % (group1_name,group2_name,float(p_rate),p_force))
		corrected_rate = str(p_rate * 100)
...
def determine_indexchain(atomstring):	#For the creation of new pull groups, the atomnumber is dependent on the respective chain of the structure, this function will determine the *.itp enumeration based on the respective chain
	pdbfile = open("gromacs\step3_input.pdb")
	atomls = []
...
module load impi/2018.4.274-iccifort-2019.1.144-GCC-8.2.0-2.31.1\n\n\n''' % (bashdata[8],bashdata[0],bashdata[7],bashdata[5],bashdata[6],bashdata[1],bashdata[3],bashdata[2],bashdata[4],iterations,iterations))
		...
		bash_new.write('''IT=1
name=%s_$IT
gmx grompp -f step4.0_minimization.mdp -c step3_input.gro -r step3_input.gro -n index.ndx -o step4.0_$IT.tpr -p topol.top;
...
''' % (bashdata[2],bashdata[3],bashdata[4],bashdata[5],bashdata[1],simname,bashdata[6],bashdata[7]))
		...
		bash_new.write('''module load cuda/10.2.89
...
'''  gmx grompp -f step4.0_minimization.mdp -c step3_input.gro -r step3_input.gro -n index.ndx -o step4.0_$IT.tpr -p topol.top;
  ...
  gmx grompp -f %s.mdp -c step4.1_$name.gro -r step4.1_$name.gro -o $name.tpr -n index.ndx -p topol.top;
  ''' % (simname,simname))
		bash_new.write(r"gmx mdrun -nt 18 -nb gpu -bonded cpu -update gpu -pme gpu -nstlist 100 -deffnm $name -dlb yes -cpo ${name}_$((xtcS + 1)) -maxh "+"%s -nsteps $nsteps -v >& log;" % (time_init)+"\n else\n  ")
		bash_new.write(r"gmx mdrun -nt 18 -nb gpu -bonded cpu -update gpu -pme gpu -nstlist 100 -deffnm $name -dlb yes -cpi ${name}_${xtcS} -cpo ${name}_$((xtcS + 1)) -noappend -maxh "+"%s -nsteps $nsteps -v >& log\n" % (time_sim))
		bash_new.write(''' fi
 #Re-launch script
 sleep 120
 sbatch $scriptN
