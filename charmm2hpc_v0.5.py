...
- THIS SCRIPT GENERATES ITERATIVE BASH SCRIPTS FOR SIMULATIONS BASED ON CHARMM-GUI OUTPUT
- THIS SCRIPT DOES NOT MODIFY EQUILIBRATION ASIDE FROM BERENDSEN THERMOSTAT!
- ALWAYS CHECK THE LOG-FILE TO SEE IF YOUR INPUTS WERE RECEIVED AS INTENDED!
- ALWAYS CHECK THE INVOLVED FILES!
- BY DEFAULT, OUTPUT FOR PRODUCTION RUNS WILL ONLY BE OUTOUT IN XTC-FILE, NOT TRR
- IF GENERATION SUCCESSFUL, "EXECUTION COMPLETE" IS DISPLAYED

The script has four modes:
taurus: 	BULLCLUSTER Dresden
hildi:		CLUSTER Lipzig hildiknecht
umbrella:	umbrella mode for-non-SLURM based run
default:	just correction of thermostats

Modify the INPUT FILE to your liking and put in all the settings, then copy the charmm2hpc*.py file and the charmm2hpc.inp file to your simulation folder (not the gromacs/ dir)
run the bash scripts from the gromacs/ dir on the machine.
'''
import os

version = "charmm2hpc.py v0.51 04/2020 for Python 3+"
logfile = open("charmm2hpc.log","w+",newline="\n")
...
#Input for PULL-Runs
biased		= 		N									#put Y or N, N skips the umbrella inclusion
...
pullrate	= 		0.001
pullforce	= 		1000
compressed	= 		Y									#put Y or N, if Y umbrella output will then only be written to *.xtc file
...
node_number =		20									#20 should be sufficient for most systems <300k atoms, benchmark this based on system size! Run fails if too many threads for too small systems!
node_taskss =		24									#change according to partition, for haswell:24
nodetype	=		haswell								#put haswell for taurus, sandy does not exist anymore
project		=		p_membranproteine

#Input for HILDI
username	=		user-name									
interval/len=		2:00:00								#interval for staggered simulation (sims in chunks are easier to handle)
...
if bias == True:	#read the pull block only when a "Y" is put into the biased row, perform consistency check
...
if preset.startswith("hildi")==True:
	bashdata = []
	for item in input_data[20:]: bashdata.append(item)
	logfile.write("\nHILDI run detected. Loading HPC section:\n")
	logfile.write(str(bashdata)+"\n\n")

#FUNCTION SECTION
'''
LINEBREAK CONVERSION DOESNT WORK FOR PYTHON 3+
...
def rewrite_step4_1(biased):			#alter the ste4.1 default thermostat to berendsen!
	separator()
	eq_mdp=open("gromacs\step4.1_equilibration.mdp", "r+")
	eq_new=open("gromacs\step4.1_%s.mdp" % (simname),"w+",newline="\n")
...
def rewrite_step5(biased=False):		#rewrite the *.mdp file based on the inputs provided
	separator()
	logfile.write("Function rewrite_step5 called. Rewriting step5 based on BIAS = %s\n" % (str(biased)))
	prod_mdp=open("gromacs\step5_production.mdp", "r+")
	prod_new=open("gromacs\%s.mdp" % (simname), "w+",newline="\n")
	compressed = True
...
def determine_indexchain(atomstring):	#For the creation of new pull groups, the atomnumber is dependent on the respective chain of the structure, this function will determine the *.itp enumeration based on the respective chain
...
def rewrite_topol():	#For Pull runs, a new topology file will be generated that loads the corresponding *.itp files
	...
	top_new=open("gromacs/topol_pull.top","w+",newline="\n")
	line_nr = 0
	for line in top:
		line_nr += 1
...
def rewrite_ndx():		#For Pull runs, generate a new NDX file with the two new groups
	...
	ind_new = open("gromacs/index_pull.ndx","w+",newline="\n")
	for line in ind: 
...
def generate_itp(name,atoms,chain):	#For Pull runs, generate a new ITP file for the particular group! Check the chain and adjust!
	...
	logfile.write(atoms)
	logfile.write("\n")
	itp_new = open("gromacs/toppar/%s.itp" % (name),"w+",newline="\n")
	pdbfile = open("gromacs\step3_charmm2gmx.pdb")
	#write header first
...
def generate_bash(hpc_type,iterations):
	...
	bash_new=open("gromacs/%s.sh" % (simname), "w+",newline="\n")	
	if hpc_type.startswith("taurus")==True:	#add header section for TAURUS
		bash_new.write('''#!/bin/bash
#SBATCH -A %s #project
#SBATCH -J %s #user
#SBATCH -p %s # you can select currently only haswell "https://doc.zih.tu-dresden.de/hpc-wiki/bin/view/Compendium/SystemTaurus#Partitions"
#SBATCH --nodes=%s #
#SBATCH --ntasks-per-node=%s #tasks/ranks per node # It should be 24 if you run on haswell
...
''' % (bashdata[2],bashdata[3],bashdata[4],bashdata[5],bashdata[1],simname,bashdata[6],bashdata[7]))
...
''')
		bash_new.write(
'''  gmx grompp -f step4.0_minimization.mdp -c step3_charmm2gmx.pdb -r step3_charmm2gmx.pdb -n index.ndx -o step4.0_$IT.tpr -p topol.top;
  gmx mdrun -nt 18 -deffnm step4.0_$IT;
  gmx grompp -f step4.1_%s.mdp -c step4.0_$IT.gro -r step4.0_$IT.gro -n index.ndx -o step4.1_$name.tpr -p topol.top;
  gmx mdrun -nt 18 -deffnm step4.1_$name;
  gmx grompp -f %s.mdp -c step4.1_$name.gro -r step4.1_$name.gro -o $name.tpr -n index.ndx -p topol.top;
  ''' % (simname,simname))
...
''')
		...
		bash_new.write('''IT=1
name=%s_$IT
gmx mdrun -nt 4 -deffnm step4.0_$IT;
gmx grompp -f step4.1_%s.mdp -c step4.0_$IT.gro -r step4.0_$IT.gro -n index.ndx -o step4.1_$name.tpr -p topol.top;
gmx mdrun -nt 4 -deffnm step4.1_$name;
gmx grompp -f %s.mdp -c step4.1_$name.gro -r step4.1_$name.gro -o $name.tpr -n index.ndx -p topol.top;
...
''' % (simname,simname))
	...
	if iterations > 1:
		...
		for i in range(1,iterations+1):
			...
			bash_it=open("gromacs/%s_%d.sh" % (simname,i),"w+",newline="\n")
